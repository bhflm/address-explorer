...
export function NftGallery({ address }: Props) {
  ...
  const renderFixedNfts = () => {
    if (nfts) {
      return (nfts.slice(start, end).map((nft, i) => (
  ...
  const renderGalleryButtons = () => {
    ...
    if (nfts) {
      ...
      const nextPageShowIndex = (currentPage * nftsPerPage + 1) > nfts.length ? null : currentPage + 1;
  
      return ( 
        <div className="flex justify-center mt-4">
          <Button className="border-0.5 m-4 sm:p-2 sm:w-1/3 sm:m-4" onClick={handlePrevPage} disabled={currentPage === 1}>
            Previous {previousPageShowIndex}
          </Button>
          <Button className="border-0.5 m-4 sm:w-1/3 sm:m-4" onClick={handleNextPage} disabled={end >= nfts.length}>
